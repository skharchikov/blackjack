use crate::domain::{
    engine::{
        ...
        error::CommandError,
        event::{
            outcome::{GameResult, PayoutMultiplier, Payout, PlayerOutcome, PlayerResult},
    ...
    fn handle(
        ...
        let player_results: Vec<PlayerResult> = state
            ...
            .map(|p| {
                let (outcome, multiplier) =
                    settle_player(p, &state.dealer.hand, dealer_busted);
        ...
        Ok(vec![
            EventPayload::GameFinished {
                result: GameResult { player_results, dealer_busted },
            },
...
mod tests {
    ...
    fn state_at_payouts(player_ranks: Vec<Rank>, dealer_ranks: Vec<Rank>, bet: u32) -> (GameState, PlayerId) {
        let pid = PlayerId::new();
        let shoe = crate::domain::Shoe::shuffled();
        let mut state = GameState::new_with_balance(GameId::new(), shoe, vec![(pid, 1000)], DealerId::new());
        state.phase = Phase::Payouts;
        state.players[0].bet = Some(bet);
        for r in player_ranks { state.players[0].hand.add_card(card(r)); }
        for r in dealer_ranks { state.dealer.hand.add_card(card(r)); }
        (state, pid)
    }
        ...
        for e in events {
            if let EventPayload::GameFinished { result } = e {
                for r in &result.player_results {
                    if r.player == pid { return r.outcome.clone(); }
                }
            }
        }
    ...
    #[test]
    fn player_wins() {
        let (state, pid) = state_at_payouts(vec![Rank::King, Rank::Nine], vec![Rank::King, Rank::Seven], 100);
    ...
    #[test]
    fn player_loses() {
        let (state, pid) = state_at_payouts(vec![Rank::King, Rank::Seven], vec![Rank::King, Rank::Nine], 100);
        let events = GameEngine::handle(&state, &settings(), &cmd()).unwrap();
    ...
    #[test]
    fn push() {
        let (state, pid) = state_at_payouts(vec![Rank::King, Rank::Eight], vec![Rank::King, Rank::Eight], 100);
        let events = GameEngine::handle(&state, &settings(), &cmd()).unwrap();
    ...
    #[test]
    fn player_blackjack() {
        let (state, pid) = state_at_payouts(vec![Rank::Ace, Rank::King], vec![Rank::King, Rank::Eight], 100);
        let events = GameEngine::handle(&state, &settings(), &cmd()).unwrap();
    ...
    fn both_blackjack_push() {
        let (state, pid) = state_at_payouts(vec![Rank::Ace, Rank::King], vec![Rank::Ace, Rank::King], 100);
        let events = GameEngine::handle(&state, &settings(), &cmd()).unwrap();
        assert_eq!(outcome_for(&events, pid), PlayerOutcome::Push);
    }
    ...
    fn dealer_busts_player_wins() {
        let (state, pid) = state_at_payouts(vec![Rank::King, Rank::Eight], vec![Rank::King, Rank::Queen, Rank::Five], 100);
        let events = GameEngine::handle(&state, &settings(), &cmd()).unwrap();
        assert_eq!(outcome_for(&events, pid), PlayerOutcome::Won);
    }
    ...
    fn player_busts_loses() {
        let (state, pid) = state_at_payouts(vec![Rank::King, Rank::Queen, Rank::Five], vec![Rank::King, Rank::Eight], 100);
        let events = GameEngine::handle(&state, &settings(), &cmd()).unwrap();
    ...
    fn payout_total_on_win() {
        ...
        if let EventPayload::GameFinished { result } = &events[0] {
            let r = result.player_results.iter().find(|r| r.player == pid).unwrap();
